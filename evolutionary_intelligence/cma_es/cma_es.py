...
def rotated_hyper_ellipsoid(x):
    return sum(numpy.sum([[x[i]**2 for i in range(j)] for j in range(1, len(x)+1)])),

if func == "levy":
    toolbox.register("evaluate", levy)
if func == "rotated":
    toolbox.register("evaluate", rotated_hyper_ellipsoid)
if func == "griewank":
...
def main(verbose=True):
    ...
    strategy = cma.Strategy(centroid=[5.0]*N, sigma=5.0, lambda_=200, mu=30)
    ...
    # x = list(range(0, strategy.lambda_ * count, strategy.lambda_))
